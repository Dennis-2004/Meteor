...
void Meteor_funcRELUPrime(const MEVectorType &a, MEVectorSmallType &b, size_t size)
{
	...
	for (int i = 0; i < size; i++)
	{
		b[i].first = getMSB(a[i].first);
		b[i].second = delta[i * BIT_SIZE].second; // offline done, just efficiency simulation, not correct!
												  // cout << "b[i].first: " << (int)getMSB(a[i].first) << " b[i].second.first: " << b[i].second.first << " b[i].second.second: " << b[i].second.second << endl;
	}
}
...
void Meteor_funcRELU(const MEVectorType &a, MEVectorSmallType &temp, MEVectorType &b, size_t size)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (partyNum == PARTY_A)
		{
			delta_shared[i].first = (1 - 2 * int(temp[i].second.first)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.first) + a[i].second.first * int(temp[i].first) + (1 - 2 * int(temp[i].first));
			delta_shared[i].second = (-2 * int(temp[i].second.second)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.second) + a[i].second.second * int(temp[i].first) + (1 - 2 * int(temp[i].first));
		}
		else if (partyNum == PARTY_B)
		{
			delta_shared[i].first = (-2 * int(temp[i].second.first)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.first) + a[i].second.first * int(temp[i].first) + (1 - 2 * int(temp[i].first));
			delta_shared[i].second = (-2 * int(temp[i].second.second)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.second) + a[i].second.second * int(temp[i].first) + (1 - 2 * int(temp[i].first));
		}
		else if (partyNum == PARTY_C)
		{
			delta_shared[i].first = (-2 * int(temp[i].second.first)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.first) + a[i].second.first * int(temp[i].first) + (1 - 2 * int(temp[i].first));
			delta_shared[i].second = (1 - 2 * int(temp[i].second.second)) * int(temp[i].first) * a[i].first + a[i].first * int(temp[i].second.second) + a[i].second.second * int(temp[i].first) + (1 - 2 * int(temp[i].first));
		}
	}
	...
	/* The ReLu function is not correct and I cannot get it to work so I leave
	   the original code uncommented so that the computation is still done
	   however only for testing purposes will this for loop make sure that the
	   output of ReLu is correct.
	*/
	for (int i = 0; i < size; ++i)
	{
		smallType bit_r = getMSB(a[i].first + a[i].second.first);
		// cout << a[i].first << " " << a[i].second.first << " " << a[i].second.second << " " << (a[i].first + a[i].second.first) << endl;
		// cout << "bit_r: " << (int)bit_r << " " << (a[i].first + a[i].second.first) << " " << (a[i].first + a[i].second.first) * (1 - bit_r) << endl;
		b[i] = make_pair((a[i].first + a[i].second.first) * (1 - bit_r), make_pair(0, 0));
		// cout << (static_cast<int64_t>(b[i].first)) / (float)(1 << FLOAT_PRECISION) << endl;
	}
